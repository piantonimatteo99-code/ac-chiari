...
const docTypeConfig = {
  document: { label: 'Documento di testo', icon: FileText, color: 'text-blue-600' },
  spreadsheet: { label: 'Foglio di calcolo', icon: Sheet, color: 'text-green-600' },
  presentation: { label: 'Presentazione', icon: Presentation, color: 'text-yellow-600' },
};
...
export default function ConsiglioVerbali({ canEdit }: { canEdit: boolean }) {
  ...
  const fetchFolderInfo = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/drive/consiglio-folder');
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Errore nel recupero della cartella');
  ...
  const loadFiles = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/drive/documents?folderId=${folderId}`);
      const data = await res.json();
  ...
  const handleCreateFolder = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/drive/consiglio-folder', { method: 'POST' });
      const data = await res.json();
  ...
  const handleCreateDocument = async () => {
    ...
    try {
      const res = await fetch('/api/drive/documents', {
        ...
        body: JSON.stringify({ folderId, name: newDocName.trim(), type: newDocType }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Errore nella creazione del documento');
      setIsNewDocOpen(false);
      setNewDocName('');
      if (data.file?.webViewLink) window.open(data.file.webViewLink, '_blank');
      await loadFiles();
    } catch (err: any) {
      setError(err.message);
    } finally {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Verbali – Documenti Google Drive
          </CardTitle>
          <CardDescription>
            Collega la cartella Google Drive per gestire i verbali del Consiglio.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col items-start gap-4">
          <p className="text-sm text-muted-foreground">
            Verrà creata automaticamente la cartella{' '}
            <span className="font-mono bg-muted px-1 rounded">App AC Chiari / Consiglio</span>{' '}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="mt-1">
                Documenti sincronizzati con{' '}
                <span className="font-mono text-xs">App AC Chiari / Consiglio</span>.
                Cliccando su un documento si aprirà in una nuova scheda.
              </CardDescription>
            </div>
        ...
        <CardContent>
          ...
          {isUploading && (
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4 p-3 bg-muted rounded-lg">
              ...
              Caricamento file su Google Drive in corso...
            </div>
          )}
          ...
          {!isLoadingFiles && files.length === 0 && (
            <div className="text-center py-10 text-muted-foreground">
              ...
              <p className="text-sm">Nessun verbale trovato nella cartella Drive.</p>
              {canEdit && (
      ...
      <Dialog open={isNewDocOpen} onOpenChange={setIsNewDocOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Inserisci il nome del documento. Sarà creato su Google Drive nella cartella{' '}
              <span className="font-mono text-xs">App AC Chiari / Consiglio</span>.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-2 py-2">
            <Label htmlFor="consiglio-doc-name">Nome documento</Label>
            <Input
              id="consiglio-doc-name"
              value={newDocName}
              onChange={(e) => setNewDocName(e.target.value)}
              placeholder={`Es. Verbale Consiglio...`}
              onKeyDown={(e) => {
